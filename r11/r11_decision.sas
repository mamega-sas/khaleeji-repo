...
and message.solution.customerType = 'PE' then do;
    ...
    do k = 1 to 6;
        if not missing(profile.customer_vs.failed_login_dtm_ar[k]) and abs(last_failed_login_dtm - profile.customer_vs.failed_login_dtm_ar[k]) < dhms(0,0,10,0) then failed_login_count = failed_login_count + 1;
    end;
