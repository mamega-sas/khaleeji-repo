...
API_BASE_URL = "https://sfdnp.khaleeji.bank/alertsTransaction/transactionGridDefinitionConfig" 
...
if response.status_code == 200:
    ...
    transaction_grids = data.get("items", [])
    with open("commands_and_scripts/transaction_grids.csv", mode="w", newline="", encoding="utf-8") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["ID","Name"])  # header row
        for field in transaction_grids:
            field_id = field.get("id")
            field_name = field.get("displayName")
            writer.writerow([field_id, field_name])
            writer.writerow(["displayName"])
            for config in field.get("transactionFieldConfigs",[]):
                writer.writerow([config.get("displayName")])
